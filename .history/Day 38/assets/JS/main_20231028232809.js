...
const blog = {
  ...
  calcTimePassed: function (prevDate, prevTime) {
    ...
    const currentSecond = objectDate
      .getSeconds()
      .toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
    const currentMinute = objectDate
      .getMinutes()
      .toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
    const currentHour = objectDate
      .getHours()
      .toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
    const currentDate = objectDate
      .getDate()
      .toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
    const currentMonth = objectDate
      .getMonth()
      .toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
    let currentYear = objectDate
      .getFullYear()
      .toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
    ...
    if (
      +currentDate === +seprateDate[1] &&
      +currentMonth + 1 === +seprateDate[0] &&
      +currentYear === +seprateDate[2]
    ) {
      if (
        +currentSecond >= +seprateTime[2] &&
        +currentMinute === +seprateTime[1] &&
        +currentHour === +seprateTime[0]
      ) {
        return "Vừa xong.";
      } else if (
        +currentMinute > +seprateTime[1] &&
      ...
      return +currentDate - +seprateDate[1] + " ngày.";
    else if (
      +currentMonth + 1 > +seprateDate[0] &&
      +currentYear === +seprateDate[2]
    )
      return +currentMonth + 1 - +seprateDate[0] + " tháng.";
    else if (+currentYear >= +seprateDate[2])
      return +currentYear - +seprateDate[2] + " năm.";
  },
      ...
      if (
        ...
        function createCalendar() {
          ...
          for (let i = firstDay; i > 0; i--) {
            storage += `<li class="inactive prev-month"}>${
              monthLastDate - i + 1
            }</li>`;
          }
